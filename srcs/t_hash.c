...
/*   Updated: 2020/02/06 12:54:14 by dygouin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/lem_in.h"

unsigned long	n2_collision(char *to_hash, t_salle *room,
	unsigned long hashed_s)
{
	if (room[hashed_s + 8].salle == NULL
	...
	unsigned long hashed_s)
{
	if (room[hashed_s - 5].salle == NULL
		...
		return (hashed_s + 7);
			...
			|| !ft_strcmp(room[hashed_s - 7].salle, to_hash))
		return (hashed_s - 7);
	return (n2_collision(to_hash, room, hashed_s));
}
...
unsigned long	col(char *to_hash, t_salle *room, unsigned long hashed_s)
{
	if (room[hashed_s + 1].salle == NULL
			|| !ft_strcmp(room[hashed_s + 1].salle, to_hash))
		return (hashed_s + 1);
